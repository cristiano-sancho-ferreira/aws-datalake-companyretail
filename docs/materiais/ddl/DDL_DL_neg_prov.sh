...
DROP TABLE IF EXISTS ${SpectrumDatabaseRaw}.neg_prov_flatfile;
DROP TABLE IF EXISTS ${SpectrumDatabaseDataLake}.neg_prov;
EOF
...
CREATE EXTERNAL TABLE ${SpectrumDatabaseRaw}.neg_prov_flatfile(
...
no_comprobante VARCHAR(20),
fecha_contable VARCHAR(10),
Doc_contable double precision,
Fecha_doc VARCHAR(10),
Valor_contabilizado double precision,
proveedor_cd VARCHAR(10))
ROW FORMAT DELIMITED FIELDS TERMINATED BY '|' 
LOCATION 's3://${bucketDataRaw}.${source}/neg_prov';


CREATE EXTERNAL TABLE ${SpectrumDatabaseDataLake}.neg_prov (
ejercicio VARCHAR(10),
acuerdo_cd VARCHAR(10),
prov_division_cd  VARCHAR(10),
periodo_contable double precision,
...
STORED AS PARQUET
LOCATION 's3://${bucketDataLake}/neg_prov';
    \p \echo \echo 
    
EOF

## ejecuta comando en el athena
echo creando particiones de la tabla neg_prov
QUERY_ID=$(aws athena start-query-execution --query-string "MSCK REPAIR TABLE ${SpectrumDatabaseDataLake}.neg_prov" --result-configuration "OutputLocation=${S3_PATH_APPLICATION},EncryptionConfiguration={EncryptionOption=SSE_S3}" --region us-east-1)
